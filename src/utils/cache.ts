 ...
 */
export async function getOrSet<T>(key: string, fetchFn: () => Promise<T>, ttl?: number): Promise<T> {
...
export function invalidateByPrefix(prefix: string): void {
  ...
  if (keys.length > 0) {
    cache.del(keys);
    logger.info(`[CACHE] Invalidadas ${keys.length} claves con prefijo "${prefix}"`);
  }
}
...
export function flushCache(): void {
  cache.flushAll();
  logger.info('[CACHE] Caché completo limpiado');
}
